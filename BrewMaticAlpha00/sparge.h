...
public:
    SpargeClass(OneWire * ds, UTFT *lcd){
    ...
    void SetFire(){
      Serial.println("Sparge TEMP:");
      Serial.println(tared_temp_);
      Serial.println("Sparge TEMP:");
      Serial.println(tared_temp_);
       if(are_temps_ok_){
    ...
    void DisplayLcd(void){
      ...
      lcd_->print("        SPARGE         ", 0, 250);
      lcd_->print("Target:" + targetTempString + "C ", 0, 270);
      
      String spargeTempString; 
      if (are_temps_ok_){
        spargeTempString = String(((int)(tared_temp_*10)/10.0 < 100.0 ? " " : "")) + String(tared_temp_, 2);
        lcd_->print("    " + spargeTempString + "C ", 0, 290);
      }else{
        spargeTempString = temperatureError(tempSparge, true); 
        lcd_->print("    " + spargeTempString + "     ", 0, 290);
      }     
    }

    const char sparge_backup_file[10] = {'s','p','a', 'r', 'g', 'e', '.', 't', 'x', 't'};
