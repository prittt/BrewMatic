...
#include "../ASensor/SensorCalibrator.h"
...
protected: 
	...
	MeasurementStatus _status;                  //!<  Last measuremnt status
        MeasurementStatus _prev_status = Default;     // Added by Federico Bolelli to handle errors outside the class
	...
	bool IsChanged()
	{
		if(_status != Error && isDiffer(_prev_value,_last_value))
			return true;
                ...
                // Added by Federico Bolelli to handle errors outside the class (in this case out of range errors)
		...
		if(_sensor->Measure(value))
		{
			...
			for (int i = 0; i<_eventMeasuredReceivers.Count(); i++)
				_eventMeasuredReceivers[i]->NotifySensorMeasured(this);
                    ...
                    // Added from Federico Bolelli to handle errors outside the class.
                    if(_prev_status != _status){
                        for(int i=0;i<_eventReceivers.Count();i++)
                            _eventReceivers[i]->NotifySensorHasData(this);
                    }
                }
                _prev_status = _status; // Added from Federico Bolelli to handle errors outside the class
	}
