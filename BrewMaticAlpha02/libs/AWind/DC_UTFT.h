...
public:
	...
	void DrawImage(const unsigned short *img, int x, int y, int width, int height, int scale)
	{
                if(height == 0){
                    _lcd->drawBitmap(x,y, width, width, img, scale);
                }else{
                    _lcd->drawBitmap(x,y, width, height, img, scale);
                }
	}
